...
- JSDoc says "caller short-circuits to false before calling this function" (NOT "caller passes {}")
  </verify>
...
- The normalization guard now also checks `detectionStored` is not null — if storage read failed, skip normalization too (we cannot know whether the user needs it).
  </action>
  <verify>
...
2. Confirm the catch block sets `detectionStored = null;` (NOT `{}`).
3. Confirm the isFirstInstall assignment uses a ternary: `detectionStored ? detectFirstInstall(detectionStored) : false`.
4. Confirm the normalization guard checks `detectionStored &&` before accessing `detectionStored.wordlebot_onboarded`.
5. Confirm the existing `var cached = await chrome.storage.local.get('wordlebot_cache')` line is unchanged.
6. Confirm clearCaches() still only removes ['wordlebot_cache', 'wordlebot_dict'] — wordlebot_onboarded is NOT listed.
7. Verify no console.log statements were added for detection result.
8. Trace the four scenarios mentally:
   ...
   - Storage failure: detectionStored = null -> ternary short-circuits to false -> no normalization write (guard fails on null check)
  </verify>
  <done>
Detection block is wired into loadDictionaryAndCaches before loadDictionary. window.WordleBot.isFirstInstall is set to true for fresh installs, false for existing users, and false on storage read failure (safe default per locked decision). Pre-v1.7 users are normalized with wordlebot_onboarded=true. Existing cache read logic is unchanged. No console logging added for detection.
  </done>
</task>
...
4. **Four-state correctness:**
   ...
   - Storage read failure -> detectionStored = null -> isFirstInstall = false (safe default per locked decision), no normalization write

5. **Storage failure safety:** The catch block sets `detectionStored = null` (not `{}`). The ternary `detectionStored ? detectFirstInstall(detectionStored) : false` ensures `detectFirstInstall` is never called with null/empty, and the result is always `false` on failure.
</verification>
